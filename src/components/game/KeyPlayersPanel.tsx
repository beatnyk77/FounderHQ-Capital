...
import { NPC_ROLE_LABELS } from "@/lib/game/specialists";
...
export function KeyPlayersPanel({ run }: { run: GameRun }) {
  return (
    <div className="rounded-lg border border-panel-border bg-panel/40 p-3">
      <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-text-dim">Politicians & Power</p>
          ...
          return (
            <div key={n.id} className="rounded border border-panel-border bg-background/50 px-2 py-1.5">
              <div className="flex items-center justify-between gap-1">
                <p className="text-[10px] uppercase text-text-dim">{NPC_ROLE_LABELS[n.role]}</p>
